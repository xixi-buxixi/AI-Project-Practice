...
> **统一入口声明**：普通开发轮次默认先读取 `AI_RULES_SUMMARY.md` 与 `.governance.yaml`，再按任务需要读取本文件。新项目、新阶段、流程争议或高风险变更时，AI 必须读取 `AI指导.md` 与本规范。支持斜杠命令的工具优先使用 `/grill-me`、`/grill-me-doc` 与 `/goal`；不支持时必须使用 `AI指导.md` 中的兼容提示词模拟同等流程。
...
3.  **Token 防爆校验机制 (Token Saving Guardrails)**：总览协调者 (OrchestratorAgent) 默认通过 `AI_RULES_SUMMARY.md`、`.governance.yaml`、本地自动化 Hook 脚本（零 Token）或审查 Agent 的结构化报告判定结果。遇到 P0/P1、跨模块冲突、疑难失败、安全问题、阶段切换或审查报告证据不足时，再定向阅读长规范和相关源码片段，避免“全量塞上下文”导致注意力稀释。
...
- 提倡使用设计模式（如策略模式、状态模式或查表法）重构代码，消除硬编码和冗余的分支逻辑，提高内聚性。
...
*   项目目标和所处阶段。
*   静态模块清单和清晰边界。
*   运行环境与依赖管理（如 Conda、venv、Poetry、NVM 及其命令执行前缀）。
*   核心文档的路径索引。
...
3. 必须核对根目录 `/agent.md` 中的“运行环境与依赖管理”配置。所有的编译、测试及静态检查命令均必须在配置的虚拟环境/工具链中执行。
4. 严格遵守本模块扩展点的强度和模式：E2 使用策略/适配器/工厂，E3 使用注册表/插件/能力声明，E4 使用规则管线/工作流/事件驱动。
5. 新增平台、渠道、供应商、内容类型、支付方式、AI 工具或格式支持时，优先新增独立策略/适配器/插件和注册项，不得直接修改核心流程来追加并列条件判断。
6. 每一轮构建或测试后，必须运行后置 Hook。
7. 完成本轮次后，更新局部看板 `/src/<module-name>/tasks.md` 中的“已完成总结” (Done Summary) 和验证日志。
8. 如果 `invalid_build_cycle_count` 达到 5 轮或 `infra_failure_count` 达到 3 轮，必须停止修改，向总览协调者汇报当前状态。
```
